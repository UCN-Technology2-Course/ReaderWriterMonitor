...
/*
 * If we consider access to a shared resource the objectives can be: - Readers
 * can access the shared resource only when there are no writers. - Writers can
 * access the shared resource only when there are no readers or writers. - Only
 * one thread can manipulate the state variables at a time.
 * 
 */
